<template>
  <div class="game-container">
    <canvas ref="gameCanvas" :width="dimensions.gameWidth" :height="dimensions.gameHeight"></canvas>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const drawGame = (state: any) => {
      if (context.value) {
        // Clear the canvas
        context.value.clearRect(0, 0, gameCanvas.value!.width, gameCanvas.value!.height);
        // Convert object of snakes to array and iterate over it
        Object.values(state.snakes).forEach((snake: any) => {
          snake.segments.forEach((segment: any) => {
            context.value!.fillStyle = 'lime';
            context.value!.fillRect(segment.x * 10, segment.y * 10, 10, 10);
          });
        });

        state.food.forEach((item: any) => {
          context.value!.fillStyle = 'red';
          context.value!.fillRect(item.position.x * 10, item.position.y * 10, 10, 10);
        });
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (direction) {
        socket.send(JSON.stringify({ type: 'change_direction', direction }));
      }
    };
